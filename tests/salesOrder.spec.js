...
test.describe('[Test Set] Create Sales Order - Superadmin', () => {
  ...
  test('[Test Case] Create Sales Order MT - Save Draft', async ({ page }) => {
    ...
    // Test Case: Create SO for GT Customer and Submit
  test('[Test Case] Create Sales Order GT - Submit', async ({ page }) => {
    const salesOrderPage = new SalesOrderPage(page);
    const so = salesOrderPage.selectors;
  ...
  // Test Case: Create SO for GT Customer and Save Draft
  test('[Test Case] Create Sales Order GT - Save Draft', async ({ page }) => {
    const salesOrderPage = new SalesOrderPage(page);
    const so = salesOrderPage.selectors;
...
test.describe('[Test Set] Sales Order Full Flow - Superadmin', () => {
  test.beforeEach(async ({ page }) => {
    await b2b.login(page, baseUrl, loginData.superadminUser.email, loginData.superadminUser.password);
  });
  
  test('[Test Case] Create Sales Order GT - Full Flow Approved', async ({ page }) => {
    const salesOrderPage = new SalesOrderPage(page);
    const so = salesOrderPage.selectors;
  ...
  test('[Test Case] Create Sales Order MT - Full Flow Approved', async ({ page }) => {
    const salesOrderPage = new SalesOrderPage(page);
    const so = salesOrderPage.selectors;
  ...
  test('[Test Case] Create Sales Order GT - Full Flow Rejected', async ({ page }) => {
    const salesOrderPage = new SalesOrderPage(page);
    const so = salesOrderPage.selectors;
    ...
    // Reject Sales Order
    // Read the saved sales order data from fixture file
    const dataPathReject1 = join(__dirname, './fixtures/salesOrderData.json');
    const freshDataReject1 = JSON.parse(readFileSync(dataPathReject1, 'utf-8'));
    const salesOrderIdNeedApproval = freshDataReject1.lastCreatedSalesOrder.salesOrderId;
    const referenceCodeNeedApproval = freshDataReject1.lastCreatedSalesOrder.referenceCode;
    console.log('Sales Order ID: ' + salesOrderIdNeedApproval);
    console.log('Reference Code: ' + referenceCodeNeedApproval);
    ...
    // Reject sales order
    await expect(so.needApprovalStatus).toBeVisible();
    await b2b.clickButton(page, so, 'cancelButton');

    // Cancellaton reason
    await expect(so.cancelReasonPopUp).toBeVisible();
    await so.cancelReasonPopUp.fill('Test Cancel Automation');
    await so.submitButton.click( {force: true, timeout: 5000});

    // OK on cancelled pop-up
  ...
  test('[Test Case] Create Sales Order MT - Full Flow Rejected', async ({ page }) => {
    const salesOrderPage = new SalesOrderPage(page);
    const so = salesOrderPage.selectors;
    ...
    // Reject Sales Order
    // Read the saved sales order data from fixture file
    const dataPathReject2 = join(__dirname, './fixtures/salesOrderData.json');
    const freshDataReject2 = JSON.parse(readFileSync(dataPathReject2, 'utf-8'));
    const salesOrderIdNeedApproval = freshDataReject2.lastCreatedSalesOrder.salesOrderId;
    const referenceCodeNeedApproval = freshDataReject2.lastCreatedSalesOrder.referenceCode;
    console.log('Sales Order ID: ' + salesOrderIdNeedApproval);
    console.log('Reference Code: ' + referenceCodeNeedApproval);
...
test.describe('[Test Set] Sales Order Approval/Rejection', () => {
  test.beforeEach(async ({ page }) => {
    await b2b.login(page, baseUrl, loginData.approverUser.email, loginData.approverUser.password);
  });
  ...
  test('[Test Case] Approve Sales Order', async ({ page }) => {
    ...
    // Read the saved sales order data from fixture file
    const dataPath = join(__dirname, './fixtures/salesOrderData.json');
    const freshData = JSON.parse(readFileSync(dataPath, 'utf-8'));
    const salesOrderId = freshData.lastCreatedSalesOrder.salesOrderId;
    const referenceCode = freshData.lastCreatedSalesOrder.referenceCode;
    console.log('Sales Order ID: ' + salesOrderId);
